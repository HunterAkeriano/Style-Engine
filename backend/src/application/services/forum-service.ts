...
import { ForumRepository, type ForumStatus } from '../../infrastructure/repositories/forum-repository'
...
export class ForumService {
  ...
  private async moveAttachmentsToTopic(topicId: string, attachments: ForumAttachment[]) {
    ...
    for (const att of attachments) {
      ...
      if (currentPath && currentPath.includes(path.join(this.uploadsRoot(), 'temp'))) {
        ...
        normalized.push({ ...att, url: `/uploads/forum/${topicId}/${filename}` })
      } else {
        normalized.push(att)
      }
    }
  ...
  private async deleteRemovedAttachments(topicId: string, previous: ForumAttachment[], next: ForumAttachment[]) {
    ...
    for (const att of toDelete) {
      const pathname = this.parsePathFromUrl(att.url)
      if (!pathname) continue
      const filePath = path.join(this.uploadsRoot(), pathname.replace('/uploads/forum/', ''))
      if (filePath.includes(path.join(this.uploadsRoot(), topicId)) && fs.existsSync(filePath)) {
        try {
  ...
  private serializeTopic(topic: ForumTopic) {
    const { user, ...rest } = topic.get({ plain: true }) as any
  ...
  private serializeMessage(message: ForumMessage) {
    const { user, ...rest } = message.get({ plain: true }) as any
  ...
  async listTopics(options: { page: number; limit: number; status?: ForumStatus }) {
  ...
  async createTopic(userId: string, payload: CreateTopicPayload) {
    ...
    if (normalizedAttachments.length !== payload.attachments.length || normalizedAttachments.length > 0) {
      await this.repo.updateTopic(created, { attachments: normalizedAttachments } as Partial<Attributes<ForumTopic>>)
  ...
  async editMessage(payload: EditMessagePayload) {
    ...
    if (!canEdit) throw toApiError(403, 'Editing is not allowed for this topic')
  ...
  async updateStatus(topicId: string, status: ForumStatus, actorIsAdmin: boolean) {
  ...
  async updateTopicContent(topicId: string, userId: string, patch: Partial<Pick<CreateTopicPayload, 'title' | 'description' | 'attachments'>>, isAdmin: boolean) {
    const topic = await this.repo.findTopicById(topicId)
    if (!topic) throw toApiError(404, 'Topic not found')
    ...
    const nextAttachments = patch.attachments ? await this.moveAttachmentsToTopic(topicId, patch.attachments) : topic.attachments
