...
export async function getForumTopics(
  params: { page?: number; limit?: number; status?: ForumStatus } = {},
...
export async function createForumTopic(payload: {
  ...
  const response = await api.post<{ topic: ForumTopic }>(
    "/forum/topics",
    payload,
  );
...
export async function updateForumTopic(
  ...
  const response = await api.patch<{ topic: ForumTopic }>(
    `/forum/topics/${id}`,
    payload,
  );
  return response.data.topic;
...
export async function changeForumTopicStatus(
  id: string,
  status: ForumStatus,
): Promise<ForumTopic> {
  const response = await api.patch<{ topic: ForumTopic }>(
    `/forum/topics/${id}/status`,
    { status },
  );
  return response.data.topic;
}
...
export async function unpinForumTopic(id: string): Promise<ForumTopic> {
  const response = await api.delete<{ topic: ForumTopic }>(
    `/forum/topics/${id}/pin`,
  );
  return response.data.topic;
}
...
export async function getPinnedForumTopics(limit = 6): Promise<ForumTopic[]> {
  ...
  const response = await api.get<{ topics: ForumTopic[] }>(
    `/forum/topics/pinned${query}`,
  );
...
export async function postForumMessage(
  ...
  const response = await api.post<{ topic: ForumTopic; message: ForumMessage }>(
...
export async function editForumMessage(
  ...
  const response = await api.patch<{ message: ForumMessage }>(
...
export function openForumStream(
  topicId: string,
  handlers: {
  ...
  const socket = new WebSocket(
    `${wsBase}/forum/ws?topicId=${encodeURIComponent(topicId)}`,
  );
  ...
  socket.onmessage = (event) => {
    try {
      ...
      if (payload.event === "topic-updated")
        handlers.onTopicUpdate?.(payload.data);
...
export function openNotificationStream(
  ...
  const wsBase = API_BASE.startsWith("https")
    ...
    : API_BASE.replace("http", "ws");
  const socket = new WebSocket(
    `${wsBase}/forum/ws?userId=${encodeURIComponent(userId)}`,
  );

  socket.onmessage = (event) => {
    try {
      const payload = JSON.parse(event.data);
      if (payload.event === "topic-reply")
        handlers.onTopicReply?.(payload.data);
    } catch (err) {
      handlers.onError?.(err);
    }
  };
...
export async function getUserOpenTopics(): Promise<{
  topics: Array<{ id: string; status: ForumStatus }>;
}> {
  const response = await api.get<{
    topics: Array<{ id: string; status: ForumStatus }>;
  }>("/forum/my-topics/open");
  return response.data;
}

export async function getTopicParticipants(
  topicId: string,
): Promise<{ participants: ForumUser[] }> {
  const response = await api.get<{ participants: ForumUser[] }>(
    `/forum/topics/${topicId}/participants`,
  );
  return response.data;
}
...
export async function deleteUserMessages(
  ...
  const response = await api.delete<{ success: boolean; deletedCount: number }>(
    `/forum/topics/${topicId}/messages/${userId}`,
  );
  return response.data;
}
...
export async function checkUserMuteStatus(
  topicId: string,
): Promise<{ muted: boolean; expiresAt?: string | null }> {
  const response = await api.get<{ muted: boolean; expiresAt?: string | null }>(
    `/forum/topics/${topicId}/mute-status`,
  );
  return response.data;
}
