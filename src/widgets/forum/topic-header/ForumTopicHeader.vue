<template>
  <section class="forum-topic__hero" v-if="topic">
      ...
      <div>
        ...
        <div class="forum-topic__meta">
          <span
            ...
            :class="`forum-topic__status_${topic.status}`"
          >
            {{ statusLabels[topic.status] }}
          </span>
          <span class="forum-topic__meta-item"
            >{{ t("FORUM.TOPIC.CREATED") }}:
            {{ formatDate(topic.createdAt) }}</span
          >
          <span class="forum-topic__meta-item"
            >{{ t("FORUM.TOPIC.UPDATED") }}:
            {{ formatDate(topic.lastActivityAt) }}</span
          >
    ...
    <div
      class="forum-topic__notice"
      :class="`forum-topic__notice_${topic.status}`"
    >
