  ...
  <div class="forum-page">
    <section class="forum-page__hero">
      ...
      <div class="forum-page__header">
        ...
        <div class="forum-page__header-actions">
          ...
          />
          <Button size="md" class="forum-page__create" @click="handleCreateClick">
    ...
    <Card variant="bordered" class="forum-page__card">
      ...
      >
        ...
        <template #cell-owner="{ row }">
          <div class="forum-page__owner">
            <div class="forum-page__avatar" :data-initials="getInitials(getOwner(row))">
              <img v-if="getOwner(row)?.avatarUrl" :src="getOwner(row)?.avatarUrl || ''" alt="" />
            ...
            <div>
              ...
              <div v-if="getOwner(row)?.isAdmin" class="forum-page__owner-role">{{ t('FORUM.ADMIN') }}</div>
            </div>
          </div>
        </template>
      </Table>
      ...
      <div v-if="pagination.totalPages > 1" class="forum-page__pagination">
        <Button variant="outline" size="sm" :disabled="pagination.page === 1 || loading" @click="changePage(pagination.page - 1)">
          {{ t('FORUM.PREV') }}
        </Button>
        <div class="forum-page__page-info">
          {{ t('FORUM.PAGE_OF', { page: pagination.page, total: pagination.totalPages }) }}
        </div>
        <Button
          variant="outline"
          size="sm"
...
import { Button, Card, Modal, Select, Table, type TableColumn } from '@/shared/ui'
import { useAuthStore } from '@/entities'
import { getForumTopics, type ForumStatus, type ForumTopic } from '@/shared/api/forum'
...
const columns = computed<TableColumn[]>(() => [
  ...
  { key: 'status', label: t('FORUM.TABLE.STATUS'), width: '140px' },
  { key: 'messagesCount', label: t('FORUM.TABLE.REPLIES'), align: 'center', width: '120px' },
...
function getOwner(row: ForumTopic | Record<string, unknown>): ForumTopic['owner'] {
...
function handleRowClick(payload: { row: Record<string, unknown>; index: number }) {
