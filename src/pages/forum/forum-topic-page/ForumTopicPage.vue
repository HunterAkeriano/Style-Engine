<template>
  <div class="forum-topic">
    ...
    <Card v-if="topic && editingTopic" variant="bordered" class="forum-topic__card">
      <ForumTopicForm
...
import ForumMessagesBoard, { type MessageNode } from '@/widgets/forum/messages-board/ForumMessagesBoard.vue'
...
const canEditTopic = computed(() => {
  ...
  return topic.value.status === 'open' && topic.value.owner?.id === authStore.user?.id
})
...
const replyPlaceholder = computed(() => {
  ...
  if (topic.value.status === 'closed' && !authStore.user?.isAdmin) return t('FORUM.TOPIC.CLOSED_REPLY')
  if (topic.value.status === 'in_review' && !authStore.user?.isAdmin) return t('FORUM.TOPIC.REVIEW_REPLY')
...
async function loadTopic() {
  loading.value = true
  try {
    const { topic: topicData, messages: topicMessages } = await getForumTopic(route.params.id as string)
...
async function saveTopicEdit(payload: { title: string; description: string; attachments: ForumAttachmentDraft[] }) {
...
async function sendReply(payload: { content: string; attachments: ForumAttachmentDraft[] }) {
  ...
  try {
    ...
    const { topic: updatedTopic, message } = await postForumMessage(topic.value.id, {
...
async function handleEditSubmit(payload: { id: string; content: string }) {
  if (!topic.value) return
  try {
    const updated = await editForumMessage(topic.value.id, payload.id, { content: payload.content })
...
function handleIncomingMessage(message: ForumMessage) {
  ...
  messages.value = [...messages.value, message].sort((a, b) => a.createdAt.localeCompare(b.createdAt))
}
